...
def dump_yamlable(value: Any) -> Any:
    if is_dataclass(value):
        return {key: dump_yamlable(inner) for key, inner in asdict(value).items()}
    if isinstance(value, dict):
        return {key: dump_yamlable(inner) for key, inner in value.items()}
    if isinstance(value, (list, tuple)):
        return [dump_yamlable(inner) for inner in value]
