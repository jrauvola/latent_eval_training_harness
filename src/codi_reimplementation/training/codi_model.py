...
def load_checkpoint_state(checkpoint: str) -> dict[str, torch.Tensor]:
    if checkpoint.endswith(".safetensors"):
        return load_safetensors(checkpoint)
    return torch.load(checkpoint, map_location="cpu")


class CODIRuntime(nn.Module):
    """Shared CODI wrapper for checkpoint loading, eval, and training."""
    ...
    def __init__(
        ...
        *,
        ...
        self.use_prj = runtime_config.use_prj
        if self.use_prj:
    ...
    def get_input_embedding_layer(self) -> nn.Module:
        base = self.codi.get_base_model() if hasattr(self.codi, "get_base_model") else self.codi
    ...
    def tie_weights_if_needed(self) -> None:
        if hasattr(self.codi, "tie_weights"):
            try:
                self.codi.tie_weights()
            except KeyError:
                # Some PEFT-wrapped models already have tied parameters wired up and
                # re-tying can fail on load. In that case we keep the loaded weights.
                return
        ...
        for latent_index in range(self.runtime_config.num_latent):
            ...
            for student_layer, teacher_layer in zip(
                ...
                student_selected = student_layer.gather(
                    1,
                    model_answer_position.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, student_layer.size(-1)),
                )
