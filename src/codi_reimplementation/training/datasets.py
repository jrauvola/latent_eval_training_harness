...
from codi_reimplementation.training.config import CodiDataConfig, CodiRuntimeConfig
...
class CodiSupervisedDataset(Dataset):
    def __init__(
        self,
        *,
        dataset_names: list[str],
        tokenizer: PreTrainedTokenizerBase,
        data_config: CodiDataConfig,
    ...
    def _preprocess(self, *, questions: list[str], cots: list[str], answers: list[str]) -> list[dict[str, torch.Tensor]]:
        ...
        for src, cot, ans, teacher in zip(source_ids, cot_ids, answer_ids, teacher_ids):
            ...
            if answer_prompt_ids.numel() > 0 and not self.tokenizer.decode(ans, skip_special_tokens=False).startswith(answer_prompt):
...
def make_supervised_data_module(
    *,
    ...
    train_dataset = CodiSupervisedDataset(
    ...
    return {
        ...
        "eval_dataset": None,
        "data_collator": CodiDataCollator(tokenizer=tokenizer),
    }
