...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Queue an evaluation run after the pid stored in a pid file exits"
    )
    parser.add_argument(
        "--wait-pid-file",
        required=True,
        help="Absolute path to the pid file that will eventually contain the process to wait for",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="Wait interval while polling for pid file / pid exit",
    )
...
def wait_for_pid_file(path: Path, poll_seconds: int) -> int:
    while True:
        if path.exists():
...
def main() -> None:
    ...
    wait_pid_file = Path(args.wait_pid_file).expanduser().resolve()
    ...
    print(
        f"[queue] waiting for pid file={wait_pid_file} before launching eval config={eval_config}",
        flush=True,
    )
    wait_pid = wait_for_pid_file(wait_pid_file, args.poll_seconds)
    print(f"[queue] detected pid={wait_pid} from pid file={wait_pid_file}", flush=True)
